...
class DB_Functions {
    ...
    public function getUserByNikAndPassword($nik, $password, $c_profile, $flagging) {
 
        // select userid from t_user_mobile where userid = '"+useridstr1+"' and pass='"+passhash+"' and flagging='01';nm
        $password = $this->checkhashMD5($password);
        $stmt = $this->conn->prepare("SELECT * FROM t_rst WHERE nik = ? and pass = ? and c_profile = ? and flagging= ?");
 
        $stmt->bind_param("ssss", $nik,$password,$c_profile,$flagging);
        $result = $stmt->execute();
...
if (isset($_POST['userid']) && isset($_POST['password'])) {
    ...
    $user = $db->getUserByNikAndPassword($nik, $password,$c_profile, $flagging);
    if ($user != false) {
        $user2 = $db->simpanUser($nik,$apps_name);
    ...
    if ($user2 != false) {
        ...
        $response["user"]["nik"] = $user2["userid"];
