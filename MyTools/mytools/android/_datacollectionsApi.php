...
class DB_Functions {
    ...
    public function getDataCollectionsData($date_x,$date_y,$userid){
            // select userid from t_user_mobile where userid = '"+useridstr1+"' and pass='"+passhash+"' and flagging='01';nm
            $stmt = $this->conn->prepare("SELECT * FROM t_collections where timestamp >= timestamp(?) AND timestamp <= timestamp(?) AND userid = ?");
...
if (isset($_POST['userid'])) {
    ...
    $data = $db->getDataCollectionsData($date_x,$date_y,$userid);
