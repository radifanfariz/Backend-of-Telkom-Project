...
class DB_Functions {
    ...
    public function updateCollections($userid,$service_number,$cust_name,$alamat,$status,$keterangan,$lat,$lng,$photo_url,$id) {
        // $uuid = uniqid('', true);
        // $hash = $this->hashSSHA($password);
        // $encrypted_password = $hash["encrypted"]; // encrypted password
        // $salt = $hash["salt"]; // salt
        $stmt = $this->conn->prepare("UPDATE t_collections SET userid=?,service_number=?,cust_name=?,alamat=?,status=?,keterangan=?,lat=?,lng=?,photo_url=? WHERE id=?");
        $stmt->bind_param("ssssssssss", $userid,$service_number,$cust_name,$alamat,$status,$keterangan,$lat,$lng,$photo_url,$id);
        $result = $stmt->execute();
        $stmt->close();
...
if (isset($_POST['userid'])){
    ...
    $inserState = $db->updateCollections($userid,$service_number,$cust_name,$alamat,$status,$keterangan,$lat,$lng,$photo_url,$id);
    if($inserState == true){
    ...
    $response["error"] = TRUE;
    $response["error_msg"] = "Update Data Gagal...!!!";
    echo json_encode($response);
    }
} else {
$response["error"] = TRUE;
