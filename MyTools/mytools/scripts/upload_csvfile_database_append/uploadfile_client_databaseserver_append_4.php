...
<?php
	...
	include "../../connection/connection.php";  // untuk fungsi: "$db_host", "$db_login", "$db_password"
	   $strSQL = "DROP TABLE IF EXISTS t_psbpots_backup_last";
	...
	{
	include "../../connection/connection.php";  // untuk fungsi: "$db_host", "$db_login", "$db_password"
	   $strSQL = "CREATE TABLE t_psbpots_backup_last SELECT * FROM t_psbpots";
...
<?php
	...
	$strSQL = "LOAD DATA LOCAL INFILE 'd:/xampplite/htdocs/webbasic/scripts/upload_csvfile_database_append/temporary_files/$_GET[files]' INTO TABLE t_psbpots FIELDS TERMINATED BY ',' LINES TERMINATED BY '\r\n' IGNORE 2 LINES";
...
<?php
	{
	include "../../connection/connection.php";  // untuk fungsi: "$db_host", "$db_login", "$db_password"
	$strSQL = "insert into t_psbpots_backup select * from t_psbpots ";
	$qry = mysql_query($strSQL) or die ("Query salah_5aaa");
	echo "success_5..., insert into p_tgl_upload"."<br>";
	}
?>
<!---  --->
...
<?	
	$myFile = $_SERVER["DOCUMENT_ROOT"]."/webbasic/scripts/upload_csvfile_database_append/temporary_files/$_GET[files]"; 
	unlink($myFile);
