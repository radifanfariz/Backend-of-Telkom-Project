<?php 
 ...
 $filename2 = $_POST["oldPath2"];
 $filename3 = $_POST["oldPath3"];
 $filename4 = $_POST["oldPath4"];
 
  if(file_exists($filename) || file_exists($filename2) || file_exists($filename3) || file_exists($filename4)){
 	if(isset($_FILES["image"]["name"])){
		unlink($filename);
		echo'File'.$filename.'telah dihapus';	
	}else{
		echo 'Tidak Dapat Menghapus'.$filename;
	}
	if(isset($_FILES["image2"]["name"])){
		unlink($filename2);
		echo'File'.$filename2.'telah dihapus';	
	}else{
		echo 'Tidak Dapat Menghapus'.$filename2;
	}
	if(isset($_FILES["image3"]["name"])){
		unlink($filename3);
		echo'File'.$filename3.'telah dihapus';	
	}else{
		echo 'Tidak Dapat Menghapus'.$filename3;
	}
	if(isset($_FILES["image4"]["name"])){
		unlink($filename4);
		echo'File'.$filename4.'telah dihapus';	
	}else{
		echo 'Tidak Dapat Menghapus'.$filename4;
	}
 }
 ...
 if(isset($_POST["idx"]) and isset($_FILES["image"]["name"])){
